...
template <class IT, class NT> class Columnar {
  ...
  size_t rank;
  size_t size;
  std::unique_ptr<size_t[]> permutation;
  std::shared_ptr<IT> dims;
  detail::Values<NT> values;

public:
  static constexpr bool has_values = !std::is_void<NT>::value;

  Columnar(const Columnar<IT, NT> &other, std::unique_ptr<size_t[]> permutation)
      : rank(other.rank), size(other.size), permutation(std::move(permutation)),
  ...
  static Columnar<IT, NT> fromCOO(const std::vector<std::array<IT, Rank>> &coo,
    ...
    for (size_t i = 0; i < size; i++) {
      ...
      if constexpr (!std::is_void<NT>::value) {
        result.values.data.get()[i] = init(coo[i]);
      }
    }
    return result;
  }
  ...
  IT *dim(size_t i) {
    if (permutation) {
      return &dims.get()[size * permutation[i]];
    } else {
      return &dims.get()[size * i];
    }
  }

  NT *val(size_t i) {
    if (permutation) {
      return &values.data.get()[permutation[i]];
    } else {
      return &values.data.get()[i];
    }
  }
